...
costs real money; see router.build_providers). The reported solve time is QPU
access time, not wall clock: network latency to Leap would otherwise swamp the
microsecond-scale anneal. The race winner is still decided by arrival order.
"""
...
def _get_sampler():
    """Build the embedding-composite sampler once (Leap handshake is slow)."""
    global _sampler
    with _sampler_lock:
        if _sampler is None:
            try:
